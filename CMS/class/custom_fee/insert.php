 <?php  

  include '../config/db.php';  
 if(!empty($_POST))  
 {  
      $output = '';  
      $message = '';  
       $class_custom = '';
 foreach($_POST["custom_class"] as $row)
 {
  $class_custom .= $row . ', ';
 }
 $class_custom = substr($class_custom, 0, -2);
      $memo = mysqli_real_escape_string($connect, $_POST["memo"]);  
     
    $custom_fee = mysqli_real_escape_string($connect, $_POST["custom_fee"]);    
    $status = mysqli_real_escape_string($connect, $_POST["status"]);    
    $due_date = mysqli_real_escape_string($connect, $_POST["due_date"]);    
      if($_POST["employee_id"] != '')  
      {  
           $query = "  
           UPDATE custom_fee   
           SET memo='$memo',
           class_id ='$class_custom',
           custom_fee = '$custom_fee',
            custom_status = '$status',  
            due_date = '$due_date'  
           WHERE custom_feeid ='".$_POST["employee_id"]."'";  
           $message = 'Data Updated';  
      }  
      else  
      {  
      
           $query = "  
           INSERT INTO custom_fee (memo, class_id, custom_fee, custom_status, due_date)  
     VALUES('$memo','$class_custom', '$custom_fee', '$status', '$due_date');  
           ";  

           $message = 'Data Inserted';  
      }  
      if(mysqli_query($connect, $query))  
      {  
        $class_custom_id = 2,3,4,5;
            $output .= '<div class="alert alert-success" id="message">' . $message . '</div>';
           $select_query = "SELECT * FROM custom_fee AS A Inner Join class AS B ON A.class_id = B.classid  WHERE A.class_id IN(2,3,4,5,6,7,8) ";  
           $result = mysqli_query($connect, $select_query);  
           
         $output .='
           <div id="display"></div>
      <div class="not">
                <table class="table table-bordered">  
                     <tr>
        <th width="10%">#S.No</th>
         <th width="30%">Roll No</th> 
       <th width="30%">Student Name</th>    
       <th width="30%">Father Name</th>
       <th width="30%">Class</th>
       <th width="30%" colspan="3">Action</th>
      </tr> ';
          
          $i=0; 
           while($row = mysqli_fetch_array($result))  
           { 
           $i++; 
          $output .='
                     <tr>
        <td>' . $i .'</td>
       <td>'. $row["classid"] .'</td>
       <td>'. $row["class_name"] .'</td>
       <td>'. $row["memo"] .'</td>
       <td>'. $row["due_date"] .'</td>
       <td><input type="button" name="view" value="view" id="'. $row["yfeeid"] .'" class="btn btn-info btn-sm view_data" /></td>
       <td><button type="button" name="delete_btn" id="'. $row["yfeeid"] .'" class="btn btn-sm btn-danger btn_delete"><i class="fa fa-trash"></i></button></td> 
       <td><input type="button" name="edit" value="Edit" id="'. $row["yfeeid"] .'" class="btn btn-info btn-sm edit_data" /></td>
      </tr>
       ';
       }  
      $output .=  '</table></div>';
      }  
 }  
echo $output;
 ?>

<script type="text/javascript">
  $( document ).ready(function(){
            $('#message').fadeIn('slow', function(){
               $('#message').delay(2000).fadeOut(); 
            });
        });
</script>
  <script type="text/javascript">
  function fill(Value) {
 
   //Assigning value to "search" div in "search.php" file.
 
   $('#search').val(Value);
 
   //Hiding "display" div in "search.php" file.
}
    $(document).ready(function() {
 
   //On pressing a key on "Search box" in "search.php" file. This function will be called.
 
   $("#search").keyup(function() {
 
       //Assigning search box value to javascript variable named as "name".
 
       var name = $('#search').val();


 
       //Validating, if "name" is empty.
 
       if (name == "") {
 
           //Assigning empty value to "display" div in "search.php" file.
 
           $("#display").html("");
            $('.not').show(); 
       }
 
       //If name is not empty.
 
       else {
 
           //AJAX is called.
 
           $.ajax({
 
               //AJAX type is "Post".
 
               type: "POST",
 
               //Data will be sent to "ajax.php".
 
               url: "class/ajax_sql_exams/search_ajax.php",
 
               //Data, that will be sent to "ajax.php".
 
               data: {
 
                   //Assigning value of "name" into "search" variable.
 
                   search: name
 
               },
 
               //If result found, this funtion will be called.
 
               success: function(html) {
 
                   //Assigning result to "display" div in "search.php" file.
      
              $("#display").html(html).show();      
               $('.not').hide();  
               }
 
           });
 
       }
 
   });
 
});
 </script>